  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-lg font-semibold text-text-primary">{menu.name}</h2>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button type="button" onClick={() => onExportGuest(guestLang)} className="rounded-lg border border-border px-3 py-1.5 text-sm text-text-secondary transition hover:bg-bg-elevated">
            {t('menu.exportGuest')}
          </button>
          <button type="button" onClick={onExportInternal} className="rounded-lg border border-border px-3 py-1.5 text-sm text-text-secondary transition hover:bg-bg-elevated">
            {t('menu.exportInternalPdf')}
          </button>
          <button type="button" onClick={onExportCsv} className="rounded-lg border border-border px-3 py-1.5 text-sm text-text-secondary transition hover:bg-bg-elevated">
            {t('menu.exportCsv')}
          </button>
        </div>
      </div>
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div className="rounded-lg border border-border bg-bg-card/60 p-3">
          <p className="text-xs text-text-secondary">{t('menu.avgCostPct')}</p>
          <p className="text-lg font-medium text-text-primary">
            {analytics.avgCostPct !== null ? formatPercent(analytics.avgCostPct, locale) : t('menu.none')}
          </p>
        </div>
        <div className="rounded-lg border border-border bg-bg-card/60 p-3">
          <p className="text-xs text-text-secondary">{t('menu.marginSpread')}</p>
          <p className="text-lg font-medium text-text-primary">
        ...
        <div className="rounded-lg border border-border bg-bg-card/60 p-3">
          <p className="text-xs text-text-secondary">{t('menu.worstOffender')}</p>
          <p className="text-lg font-medium text-text-primary">{worstName}</p>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-border/70">
              {sortedRows.map((row, index) => {
                ...
                return (
                  <tr key={row.recipe.id} className={FLAG_ROW_TINT[row.flag] || 'bg-bg-card/40'}>
                    <td className="px-2 py-3">
                      <span className={`inline-block h-3 w-3 rounded-full ${FLAG_DOT[row.flag]}`} aria-label={row.flag} />
                    </td>
                    <td className="px-4 py-3 font-medium text-text-primary">{row.recipe.name}</td>
                    <td className="px-4 py-3 text-text-secondary">
                      {row.priceGross !== null ? formatEur(row.priceGross, locale) : t('menu.unpriced')}
                    </td>
                    <td className="px-4 py-3 text-margin-good">{formatEur(row.pourCost, locale)}</td>
