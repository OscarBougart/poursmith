      ...
      <form onSubmit={(e) => void handleSubmit(e)} noValidate>
        <Field label={t('settings.targetCostPct')} htmlFor="settings-target" error={error ?? undefined}>
          <input
            ...
            className="w-full rounded-lg border border-border bg-bg-primary px-3 py-2 text-text-primary outline-none focus:border-accent"
          />
        </Field>
        <button
          type="submit"
          disabled={pending}
          className="w-full rounded-lg bg-accent px-4 py-2.5 text-sm font-medium text-white transition hover:bg-accent-d1 disabled:opacity-60"
        >
